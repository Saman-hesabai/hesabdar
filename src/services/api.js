...
export async function addCustomer(storeId,name,phone='',notes=''){ const {data:{user}}=await supabase.auth.getUser(); const payload={store_id:storeId,full_name:name,phone,notes,created_by:user?.id,is_deleted:false}; const {data,error}=await supabase.from('customers').insert(payload).select().single(); if(error) throw error; return data }
